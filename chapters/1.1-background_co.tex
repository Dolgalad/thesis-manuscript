...
A formulation is said to be \emph{compact} if the number of variables and constraints is polynomial in the size of the input~\cite{yannakakis_expressing_1988,carr_compact_2000}. This assumption is important for the methods developed in this thesis, since the variable-constraint graph must be represented explicitly. We therefore focus mainly on problems that admit compact BILP formulations. Compactness, however, does not imply that every constraint is small: a compact formulation may still contain constraints involving many variables. This distinction becomes important in the canonical MRF construction of Section~\ref{subsec:canonical-feasibility-cost-param} and in the approximate factor-graph models of Chapter~\ref{chapter:5_feature-to-factor_mappings_for_approximate_mrf_inference}.

\subsubsection{Linear programming relaxations and solver-state quantities}
Given a BILP with feasible set $\mathcal{X}=\{0,1\}^n$, its linear programming (LP) relaxation is obtained by replacing the integrality constraints with continuous bounds:
\begin{equation}
...
Because exact ILP solving can be computationally expensive, modern solvers rely heavily on primal heuristics to quickly construct feasible incumbent solutions. Examples include diving heuristics, rounding and repair procedures such as the feasibility pump~\cite{fischetti_feasibility_2005}, and neighborhood-based methods such as RINS~\cite{danna_exploring_2005} and Variable Neighborhood Search (VNS)~\cite{mladenovic_variable_1997,hansen_variable_2008}. These methods are relevant to this thesis because they highlight a central difficulty: a useful prediction is not merely a vector of independent variable scores, but a coherent assignment that satisfies the constraints. Chapter~\ref{chapter:3_differentiable_mrf_inference_for_BILPs} addresses this difficulty with MRF-based marginal inference and feasibility-preserving decoding, while Chapter~\ref{chapter:4_denoising_and_mrf_inference} studies denoising and flow-based refinement dynamics.
...
It is important to distinguish this representation from the factor graphs introduced in Section~\ref{sec:mrf_intro}. The variable-constraint graph is an input representation of the ILP, typically used by GNN encoders. A factor graph, by contrast, is used in Section~\ref{sec:mrf_intro} to define the factorization of a probability distribution or energy model over solution variables, and later becomes the structured prediction layer in Chapter~\ref{chapter:3_differentiable_mrf_inference_for_BILPs}. In many of our models the two graphs are closely related, because constraints in the ILP will later give rise to factors in the MRF, but they play different roles.
...
\label{subsec:background_representative_problems}
