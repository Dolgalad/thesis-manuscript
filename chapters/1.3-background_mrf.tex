...
\input{resources/tikz/chapter_1/mrf/two_spin_ising_example}
\end{example}
...
\input{resources/tikz/chapter_1/mrf/four_spin_ising_example}
\end{example}
...
Factor graphs provide an alternative representation of graphical models that makes the factorization structure of the distribution explicit. Formally, a factor graph is a bipartite graph $H=(V \cup F,E_H)$ consisting of variable nodes $V$ corresponding to random variables $\{X_v : v\in V\}$ and factor nodes $F$ corresponding to potential functions $\{\psi_f : f\in F\}$. An edge $(v,f)\in E_H$ indicates that variable $X_v$ participates in factor $\psi_f(x_f)$. The joint distribution over all variables then factorizes as
\[
p(x) = \frac{1}{Z}\prod_{f\in F}\psi_f(x_f).
\]
...
\input{resources/tikz/chapter_1/mrf/chain_factor_graph_example}
...
\input{resources/tikz/chapter_1/mrf/learning_augmented_bp}
...
\input{resources/tikz/chapter_1/mrf/bp_message_passing}
