...
\begin{scope}[xshift=3.8cm]
  \node[align=center,anchor=south] at (0,1.8) {\bf SC dive};
  ...
  % Sets (bottom row)
  \node[set,fill=gray!10]  (S1) at (-1.2,0.0) {\small $\SCSET_1$};
  \node[set,fill=green!20] (S2) at ( 0.0,0.0) {\small $\SCSET_2$};
  \node[set,fill=gray!10]  (S3) at ( 1.2,0.0) {\small $\SCSET_3$};
  ...
  \begin{scope}[yshift=-2.0cm]
    \node[draw,rounded corners,inner sep=3pt,fill=white] (leg) at (0,0) {
      \begin{tabular}{@{}l@{}}
        \tikz{\node[align=left,anchor=west] at (2.0,0.0) {\small \textbf{add} $\SCSET_2$  \small (unique cover of $a_5$)};} \\
        \tikz{\node[align=left,anchor=east] at (-2.0,0.0) {\small \textbf{discard} $\SCSET_1$  \small (no new items)};}
      \end{tabular}
    };
  \end{scope}
\end{scope}
...
\end{tikzpicture}
\caption{Diving steps: (left) MIS—select a vertex and remove its neighbors; (right) SC—add a set with unique coverage and discard sets adding no new items.}
\end{figure}