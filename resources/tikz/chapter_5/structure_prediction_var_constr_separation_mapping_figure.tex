...
\node[vvec] (hi) at (0,1.2) {$\EMB_i$};
\node[vvec] (hj) at (0,0.0) {$\EMB_j$};

\node[cvec] (hc1) at (0,-1.2) {$\EMB_{c_1}$};
\node[cvec] (hc2) at (0,-2.4) {$\EMB_{c_2}$};
...
% first row
\node[tiny] (ha11) at (0.45,1.7) {$\EMB_{c_1}^{(1)}$};
\node[tiny] (ha21) at (1.35,1.7) {$\EMB_{c_2}^{(1)}$};

% second row
\node[tiny] at (0.45,0.95) {$\EMB_{c_1}^{(2)}$};
\node[tiny] at (1.35,0.95) {$\EMB_{c_2}^{(2)}$};
...
% last row
\node[tiny] at (0.45,-0.65) {$\EMB_{c_1}^{(d)}$};
\node[tiny] at (1.35,-0.65) {$\EMB_{c_2}^{(d)}$};
...
\node[vvec] (ohi) at (0,0.8) {$\EMB_i$};
\node[vvec] (ohj) at (0,-0.3) {$\EMB_j$};

\node[ovec] (ohc1) at (0,-1.6) {$\underset{c \in \mathcal N(\alpha)\cap C}{\square} \EMB_c$};
...
\end{tikzpicture}
\caption{Separating variable and constraint features in the parameter prediction head. Variable embeddings \(\EMB_i\) and \(\EMB_j\) are kept as distinct inputs, while only neighboring constraint embeddings are aggregated coordinate-wise into a last input.}
