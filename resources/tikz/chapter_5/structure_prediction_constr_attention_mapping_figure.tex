...
%\node[ovec] (hcagg) at (6.1,-1.8) {$\EMB^c_{ij}$};
...
\node[ovec] (ohc)  at (0,-1.8) {$\EMB^c_{ij}$};
...
\end{tikzpicture}
\caption{Attention-based aggregation of constraint embeddings. Variable embeddings \(\EMB_i\) and \(\EMB_j\) remain distinct inputs, while neighboring constraint embeddings are combined into \(\EMB^c_{ij}\) by a weighted sum with attention coefficients \(\beta_{ij,c}\) predicted from \((\EMB_i,\EMB_j,\EMB_c)\).}
