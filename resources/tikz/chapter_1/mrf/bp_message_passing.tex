...
\begin{tikzpicture}[
  ...
  factor/.style={rectangle,draw,rounded corners=0pt,minimum width=16pt,minimum height=16pt,inner sep=2pt},
...
\node[factor] (u3) at (8,2)   {$\FACTOR_3$}; % keep as provided
...
% Unary factors
\draw[msgVF,bend left=28] (x1) to node[msglabel,pos=0.5,yshift=5pt] {$m_{1\rightarrow \FACTOR_1}$} (u1);
\draw[msgFV,bend left=28] (u1) to node[msglabel,pos=0.5,yshift=-5pt] {$m_{\FACTOR_1\rightarrow 1}$} (x1);

\draw[msgVF,bend left=28] (x2) to node[msglabel,pos=0.5,yshift=5pt] {$m_{2\rightarrow \FACTOR_2}$} (u2);
\draw[msgFV,bend left=28] (u2) to node[msglabel,pos=0.5,yshift=-5pt] {$m_{\FACTOR_2\rightarrow 2}$} (x2);

\draw[msgVF,bend left=28] (x3) to node[msglabel,pos=0.5,yshift=5pt] {$m_{3\rightarrow \FACTOR_3}$} (u3);
\draw[msgFV,bend left=28] (u3) to node[msglabel,pos=0.5,yshift=-5pt] {$m_{\FACTOR_3\rightarrow 3}$} (x3);
...
% --- Legend
\node[anchor=north] at (4,-2.5) {%
\begin{tabular}{@{}c c@{}}
    \tikz \draw[msgVF] (0,0.6) -- ++(1.1,0) node[right,black,font=\scriptsize] {variable--to--factor $\,m_{i\rightarrow \FACTOR}^{(t)}$}; & 
    \tikz \draw[msgFV] (0,0.1) -- ++(1.1,0) node[right,black,font=\scriptsize] {factor--to--variable $\,m_{\FACTOR\rightarrow i}^{(t)}$}; \\
\end{tabular}%
};
\end{tikzpicture}
