...
\draw (-0.8,-0.4) -- (0.8,-0.4);
\draw (-0.8,-0.8) -- (0.8,-0.8);
\node[lbl] at (0,-2.1) {$H^{(l)} \in \mathbb{R}^{n\times d}$};

% Q K V projections
\node[opblock] (qproj) at (3.2,1.6) {$Q = H^{(l)}W_Q$};
\node[opblock] (kproj) at (3.2,0.0) {$K = H^{(l)}W_K$};
\node[opblock] (vproj) at (3.2,-1.6) {$V = H^{(l)}W_V$};
...
\node[block, minimum width=35mm, minimum height=16mm] (attn) at (7.4,0){$\operatorname{Attn}(H^{(l)})$};
...
\node[lbl] at (21.0,-2.1) {$H^{(l+1)} \in \mathbb{R}^{n\times d}$};

\end{tikzpicture}}
\caption{Generic transformer layer with a single self-attention head. Starting from input embeddings $H^{(l)}$, learned projections produce queries, keys, and values. Self-attention aggregates information across the input elements, after which residual connections, normalization, and a position-wise feed-forward network produce the updated embeddings $H^{(l+1)}$.}
